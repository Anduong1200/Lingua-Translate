        ...
        <div className="mx-auto flex max-w-5xl flex-col gap-6 pb-8 transition-colors duration-300">
            ...
            <section className="glass border border-white/60 dark:border-slate-800/80 bg-white/70 dark:bg-slate-900/70 p-7 custom-shadow rounded-[2rem] backdrop-blur-md">
                ...
                <div className="space-y-4">
                    {user ? (
                        <div className="flex items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/30">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs font-black text-amber-600 mt-1 uppercase tracking-wider">{user.plan} PLAN</div>
                                </div>
                            </div>
                            <button
                                onClick={logout}
                                className="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
                            >
                                Đăng xuất
                            </button>
                        </div>
                    ) : (
                        <div className="p-5 rounded-xl border border-blue-200 dark:border-blue-900/30 bg-blue-50/50 dark:bg-blue-900/10">
                            <p className="text-sm text-slate-600 dark:text-slate-300 mb-4 font-medium">
                                Đăng nhập bằng tài khoản dự án hoặc Google để lưu trữ đám mây và truy cập các từ điển cao cấp.
                            </p>
                            <div className="flex gap-3">
                                <button
                                    onClick={loginWithGoogle}
                                    disabled={isLoggingIn}
                                    className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50"
                                >
                                ...
                                <button
                                    onClick={() => loginWithEmail('user@hanora.app')}
                                    disabled={isLoggingIn}
                                    className="flex items-center gap-2 px-4 py-2 bg-[#006b5f] text-white rounded-lg shadow-sm text-sm font-bold hover:bg-teal-700 disabled:opacity-50"
                                >
                                    Đăng nhập Email
                                </button>
                            </div>
